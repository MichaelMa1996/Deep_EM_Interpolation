...
class network(nn.Module):
    def __init__(self):
        ...
        self.linear4 = nn.Linear(in_features=60, out_features=849)
...
added_noise = np.random.normal(noise_mean, noise_std, size= Dataset0.shape)
Dataset1 = Dataset0+added_noise
PMU_for_training = 99 # number of PMU is selected while other converted to AMI data
...
smart_meter_index = list(range(15, 654, Smart_mater_resolution))
all_time_step_index  = list(range(15, 654, 1))
...
for i in smart_meter_index:
    ...
    y_train_sub = Dataset0[i, PMU_for_training:]
...
for i in interpolate_remaining:
    ...
    y_test_sub = Dataset0[i, PMU_for_training:]
...
#############################################################
snr = 10*math.log10(np.average(signaltonoise(np.array(X_train))))
print(snr)
###########################################################
X_train_total = torch.from_numpy(np.array(X_train).reshape(len(smart_meter_index),x_train_datasize))
y_train_total = torch.from_numpy(np.array(y_train).reshape(len(smart_meter_index),y_train_datasize))
...
iteration_step = 100
y_test_predict_tensor = torch.empty(1,14) # initialize the predit
...
for iter in range(iteration_step):
    ...
    for epoch in range(10):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(loader, 0):
            ...
            gm = GaussianMixture(n_components=1,random_state=0).fit(np.reshape(true_noise.T,(-1,1)))
